...
import { Users, Receipt, Ticket, BookOpen, AlertCircle, ArrowRight, UserPlus, UserMinus, Shield, Settings } from 'lucide-react';
import { PieChart, Pie, Cell, ResponsiveContainer, BarChart, Bar, XAxis, YAxis, Tooltip, CartesianGrid } from 'recharts';
import { useMemo } from 'react';
import { StaggerContainer, StaggerItem } from '@/components/motion/MotionPrimitives';

const ticketTypeLabels: Record<string, string> = {
  policy_question: 'Policy', grievance: 'Grievance', harassment: 'Harassment',
  IT_security: 'IT/Security', payroll: 'Payroll', benefits: 'Benefits', other: 'Other',
};

const ticketTypeColors: Record<string, string> = {
  policy_question: 'hsl(210, 100%, 58%)', grievance: 'hsl(0, 72%, 56%)',
  harassment: 'hsl(330, 70%, 56%)', IT_security: 'hsl(280, 70%, 62%)',
  payroll: 'hsl(38, 95%, 56%)', benefits: 'hsl(155, 60%, 46%)', other: 'hsl(215, 15%, 52%)',
};

const AdminDashboard = () => {
  const { currentUser } = useAuth();
  const { claims, tickets, leaveRequests, handbook, onboardingCases, onboardingTasks, exitCases } = useData();
  ...
  const activeOnboarding = onboardingCases.filter(c => c.status !== 'complete');
  const activeExits = exitCases.filter(c => c.status !== 'closed');
  ...
  const ticketDonut = useMemo(() => {
    ...
    return Object.entries(counts).map(([type, value]) => ({
      name: ticketTypeLabels[type] || type, value,
  ...
  const claimStatusBar = useMemo(() => {
    ...
    return Object.entries(counts).map(([status, value]) => ({
      name: statusLabels[status] || status, value, fill: statusColors[status] || 'hsl(215, 15%, 52%)',
    }));
  }, [claims]);
    ...
    <StaggerContainer className="space-y-6">
      ...
      <StaggerItem>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            ...
            { icon: Receipt, label: 'Pending Claims', value: pendingClaimsOrg.length, sub: `${claims.length} total`, color: 'warning' },
            { icon: Ticket, label: 'Open Tickets', value: openTicketsAll.length, sub: `${unassignedTickets.length} unassigned`, color: 'negative' },
            { icon: UserPlus, label: 'Onboarding', value: activeOnboarding.length, sub: `${overdueTasks.length} overdue tasks`, color: 'info' },
          ].map(kpi => (
            ...
            <div className="flex items-center gap-4">
              <div className="w-40 h-40">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={ticketDonut} cx="50%" cy="50%" innerRadius={40} outerRadius={65} paddingAngle={3} dataKey="value" stroke="none">
                      {ticketDonut.map((entry, i) => <Cell key={i} fill={entry.color} />)}
                    </Pie>
                    <Tooltip contentStyle={{ background: 'hsl(220, 22%, 10%)', border: '1px solid hsl(220, 18%, 20%)', borderRadius: '12px', fontSize: '12px', color: 'hsl(210, 20%, 93%)' }} />
                  </PieChart>
                </ResponsiveContainer>
              </div>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={claimStatusBar} barSize={28} layout="vertical">
                  ...
                  <Bar dataKey="value" radius={[0, 6, 6, 0]}>
                    {claimStatusBar.map((entry, i) => <Cell key={i} fill={entry.fill} />)}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            </div>
      ...
      <StaggerItem>
        <div className="glass-card p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/tickets"><button className="btn-glass text-xs">View all <ArrowRight className="w-3 h-3" /></button></Link>
          </div>
      ...
      <StaggerItem>
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
          {[
            { to: '/employees', icon: Users, label: 'Employees', sub: `${activeEmployees.length} active` },
            { to: '/onboarding', icon: UserPlus, label: 'Onboarding', sub: `${activeOnboarding.length} active` },
            { to: '/exit', icon: UserMinus, label: 'Exit Mgmt', sub: `${activeExits.length} active` },
            { to: '/handbook', icon: BookOpen, label: 'Handbook', sub: `${handbook.length} sections` },
            { to: '/operations', icon: Settings, label: 'Operations', sub: 'Import & config' },
          ].map(item => (
